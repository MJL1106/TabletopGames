...
public class JaipurMetrics implements IMetricsCollection {
    ...
    public static class FirstPlayerWinRate extends AbstractMetric {

        @Override
        protected boolean _run(MetricsGameListener listener, Event e, Map<String, Object> records) {
            JaipurGameState gs = (JaipurGameState) e.state;
            int firstPlayerWin = gs.getPlayerResults()[0] == CoreConstants.GameResult.WIN_GAME ? 1 : 0;
            records.put("FirstPlayerWin", firstPlayerWin);
            return true;
        }
        ...
        public Map<String, Class<?>> getColumns(int nPlayersPerGame, Set<String> playerNames) {
            Map<String, Class<?>> columns = new HashMap<>();
            columns.put("FirstPlayerWin", Integer.class);
            return columns;
        }
    }

    public static class HandSizeAtRoundEnd extends AbstractMetric {
        ...
        protected boolean _run(MetricsGameListener listener, Event e, Map<String, Object> records) {
            ...
            for (int i = 0; i < gs.getNPlayers(); i++) {
                for (JaipurCard.GoodType type : gs.getPlayerHands().get(i).keySet()) {
                    totalCards += gs.getPlayerHands().get(i).get(type).getValue();
                }
            }
            double avgHandSize = totalCards / gs.getNPlayers();
            records.put("AvgHandSize", avgHandSize);
            return true;
        }
        ...
        public Map<String, Class<?>> getColumns(int nPlayersPerGame, Set<String> playerNames) {
            Map<String, Class<?>> columns = new HashMap<>();
            columns.put("AvgHandSize", Double.class);
            return columns;
        }
    }
}
